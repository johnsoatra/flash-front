const kh = {
  ...
  "flash hero title": (cardPrice: number) => <>
    ...
    <b>កាតបញ្ចូលទូរស័ព្ទស្មាត<span className="font-poppins">$1</span>មួយសន្លឺក</b>ជារៀងរាល់ខែក្នុងតម្លៃត្រឹមតែ<b>៛{cardPrice}</b>បុណ្ណោះ
  </>,
  "no card left": "មិនមានកាតនៅសល់ទេ។",
  "card only left": (amount: number) => {
    return (
      <>
        ចំនួនកាតនៅសល់តែ<span className="text-[4rem]">{amount}</span>សន្លឹកទៀតបុណ្ណោះ។
      </>
    );
  },
  "already ordered": "លោកអ្នកបានធ្វើការជាវរួចរាល់ហើយសម្រាប់ខែនេះ។",
  "check next month": "សូមធ្វើការត្រួតពិនិត្យម្ដងទៀតនៅខែក្រោយ។",
  ...
  "ordering processing": "កំពុងធ្វើប្រតិបត្តិការការជាវ...",
  "copy successful": "លេខកូដរបស់កាតត្រូវបានចម្លង!",
  "no transaction": "មិនទាន់មានការទូរទាត់សម្រាប់ qr code នេះនៅឡើយ",
  "cannot get config": "មិនអាចទាញយក config បាន",
  "footer notation description": (allowAmount: number) => `
    គេហទំព័រនេះមិនមែនជាអ្នកកាតស្មាតជាផ្លូវការនោះទេ។ គេហទំព័រនេះគ្រាន់តែធ្វើការលក់ជាបុណ្ដោះអាសន្ន ដោយផ្ដល់ជូនលោកអ្នកនូវ កាតបញ្ជូលទូរស័ព្ទជាមួយនឺងការបញ្ចុះតម្លៃមួយ។
    យើងផ្ដល់ជូនអ្នកត្រឹមតែ${allowAmount}កាតបុណ្ណោះក្នុងមួយខែ ហើយមនុស្សម្នាក់អាចធ្វើការជាវបានតែម្ដងបុណ្ណោះ។
  `,
  ...
  "sure close scan": () => <>
    ...
    អ្នកមិនទាន់បានចុច<span className="font-medium">ផ្ទៀងផ្ទាត់ការទូរទាត់</span>នៅឡើយ។
  </>,
  "cancel scanning": "បិទការស្កែន!",
  "delete card": "លុបកាត!",
  "click yes delete": "ពេលដែលអ្នកចុចយក\"ព្រម\"នោះពត៍មានកាតរបស់អ្នកនឹងត្រូវបានលុបចោល។",
  "getting cards": "កំពុងទាញយកកាត...",
  "fail get cards": "មិនអាចទាញយកពត៍មានកាតបាន!",
  ...
  "click verify transaction": () => <>
    ...
    បន្ទាប់មកសូមចុច<span className=" font-bold uppercase">ផ្ទៀងផ្ទាត់ការទូរទាត់</span>ដើម្បីផ្ទៀងផ្ទាត់ការបង់ប្រាក់របស់អ្នក។
  </>,
  'verify transaction': 'ផ្ទៀងផ្ទាត់ការទូរទាត់',
  'buy card': 'ទិញកាតមួយ',
  ...
  "cancel action": "លុបប្រតិបត្តិការ",
